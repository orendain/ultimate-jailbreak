...
new const PLUGIN_PREFIX[] = "Allied-Gamers"

static const g_szBallBounce[] = "kickball/bounce.wav"
static const g_szBallModel[] = "models/Allied_Gamers/ball.mdl"
...
public ShowMainMenu(id)
{
    ...
    iLen = formatex(szBuffer, sizeof szBuffer - 1, "\r[\y%s\r] \wSoccerBall^n^n", PLUGIN_PREFIX)
...
public ShowBallMenu(id)
{
    ...
    iLen = formatex(szBuffer, sizeof szBuffer - 1, "\r[\y%s\r] \wSoccer Ball^n^n", PLUGIN_PREFIX)
...
public ShowNetMenu(id)
{
    ...
    iLen = formatex(szBuffer, sizeof szBuffer - 1, "\r[\y%s\r] \wSoccer Net^n^n", PLUGIN_PREFIX)
...
public plugin_precache()
{
    ...
    headshot = precache_model("sprites/test4.spr")   
    g_iTrailSprite = precache_model("sprites/aeroblast.spr")
    g_Trail = precache_model("sprites/psybeam.spr")
...
public remove_beam(iOwner)
{
	...
	//ColorChat(iOwner, "%s had the ball last!", name)
...
KickBall(id)
{
    ...
    ColorChat(id, "Kick Velocity: ^x04%i^x01", num)
...
public Goal()
{
    ...
    if(g_Owner != 0)
    {
        ...
        ColorChat(0, "^x04%s^x01 scored a goal from^x04 %d^x01 feet away!", name, floatround( fdistance/modFeet ))
        remove_beam(g_Owner)
    ...
    ColorChat(0, "The ball will respawn in 5 seconds.")
}
