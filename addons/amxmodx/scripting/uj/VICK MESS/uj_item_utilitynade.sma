...
new const ITEM_MESSAGE[] = "Utility Nade: Right Click [Attack2] to change grenade modes.";
...
mode[playerID][GRENADE_EXPLOSIVE] = FirstEnabledMode(GRENADE_EXPLOSIVE)
mode[playerID][GRENADE_FLASHBANG] = FirstEnabledMode(GRENADE_FLASHBANG)
mode[playerID][GRENADE_SMOKEGREN] = FirstEnabledMode(GRENADE_SMOKEGREN)

resetCounter(playerID)

cl_nextusetime[playerID] = 0.0

cl_team[playerID] = CS_TEAM_UNASSIGNED
...
set_task(0.1, "task_botHamHooks", playerID)
}
}
...
if (get_bit(g_hasItem, id) && get_option(OPTION_DISPLAY_MODE_ON_DRAW) && grenade_can_be_used(NadeRace:nade))
{
